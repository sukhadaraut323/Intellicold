...
def get_actions(prediction: Dict[str, Any], distance_km: float = 0) -> List[Dict[str, str]]:
    ...
    # QUALITY-BASED URGENT ACTIONS (even if risk is low!)
    if quality < 20:
        actions.append({
            'priority': 'CRITICAL',
            'action': f'URGENT: Quality at {quality:.1f}% - Deliver immediately or reject at destination',
            'type': 'routing'
        })
        actions.append({
            'priority': 'HIGH',
            'action': 'Alert customer about quality degradation',
            'type': 'notification'
        })
    elif quality < 50:
        actions.append({
            'priority': 'HIGH',
            'action': f'Quality degrading ({quality:.1f}%) - Expedite delivery',
            'type': 'routing'
        })
        actions.append({
            'priority': 'MEDIUM',
            'action': 'Increase monitoring frequency to every 10 minutes',
            'type': 'monitoring'
        })
